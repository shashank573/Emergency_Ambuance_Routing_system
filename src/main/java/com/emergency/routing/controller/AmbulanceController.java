...
public class AmbulanceController {
    ...
    public ResponseEntity<Ambulance> updateStatus(@PathVariable Long id, @RequestBody StatusUpdateRequest request) {
        return ResponseEntity.ok(ambulanceService.updateStatus(id, request.status()));
    }

    @PutMapping("/code/{code}/status")
    public ResponseEntity<Ambulance> updateStatusByCode(@PathVariable String code, @RequestBody StatusUpdateRequest request) {
        return ResponseEntity.ok(ambulanceService.updateStatusByCode(code, request.status()));
    }
    ...
    public ResponseEntity<?> createAmbulance(@RequestBody Ambulance ambulance) {
        if (ambulanceService.existsByCode(ambulance.getCode())) {
            return ResponseEntity.badRequest().body(java.util.Collections.singletonMap("error", "Ambulance ID already exists."));
        }
        ambulance.setStatus(AmbulanceStatus.AVAILABLE);
        return ResponseEntity.status(org.springframework.http.HttpStatus.CREATED).body(ambulanceService.save(ambulance));
    }
